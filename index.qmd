...
[Redivis](https://stanford.redivis.com/datasets/a3v0-3drqa89kg). To contribute a
dataset, contact
...
html`<div style="overflow-x:auto"><table class="pb-plain-table">
  <thead><tr>
    <th>dataset</th><th>citation</th><th style="text-align:right">children</th>
    <th style="text-align:right">sessions</th><th>age range (mo)</th><th>languages</th>
  </tr></thead>
  <tbody>${manifest.datasets.map((d) => html`<tr>
    <td><a href="datasets.html">${d.name}</a></td>
    <td>${d.shortcite ?? ""}</td>
    <td style="text-align:right">${d.n_subjects.toLocaleString()}</td>
    <td style="text-align:right">${d.n_admins.toLocaleString()}</td>
    <td>${d.age_min}–${d.age_max}</td>
